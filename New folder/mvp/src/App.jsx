...
function isUniversityMember(user) {
  return !!user && ["student", "faculty", "staff"].includes(user.role);
}

function isOpsUser(user) {
  return !!user && ["operator", "admin"].includes(user.role);
}
...
function Sidebar({ page, setPage, user }) {
  ...
  const visible = items.filter(([key]) => {
    if (["student", "faculty", "staff"].includes(user.role)) {
      return ["dashboard", "map", "entry"].includes(key);
    }

    if (user.role === "visitor") {
      return ["temporary", "map"].includes(key);
    }

    if (user.role === "operator") {
      return ["dashboard", "map", "temporary", "operator", "sensors", "logs"].includes(key);
    }

    if (user.role === "admin") {
      return ["dashboard", "map", "admin", "logs"].includes(key);
    }
...
function EntryExitPage({ user, zones, setZones, sessions, setSessions, payments, setPayments, policies, addLog }) {
  ...
  function handleEntry() {
    ...
    if (!roleCanUseGate(user.role, zone)) {
